...
public class ClientHeartBeat<E extends Delayed> extends AbstractQueue<E> implements BlockingQueue<E> {
	...
	/**
	 * Creates a <tt>DelayQueue</tt> initially containing the elements of
	 * the given collection of {@link Delayed} instances.
	 * 
	 * @param c
	 ...
	 *                 are null
	 */
	public ClientHeartBeat(Collection<? extends E> c) {
		this.addAll(c);
	}
	...
	public boolean containsAndUpdate(E o) {
	...
	/**
	 * Retrieves and removes the head of this queue, waiting if necessary
	 * until an element with an expired delay is available on this queue, or
	 * the specified wait time expires.
	 * 
	 * @return the head of this queue, or <tt>null</tt> if the specified
	 *         waiting time elapses before an element with an expired delay
	 *         becomes available
	 * @throws InterruptedException
	 *                 {@inheritDoc}
	 */
	 ...
	 * <tt>null</tt> if this queue is empty. Unlike <tt>poll</tt>, if no
	 * expired elements are available in the queue, this method returns the
	 * element that will expire next, if one exists.
	 * 
	...
	/**
	 * Atomically removes all of the elements from this delay queue. The
	 * queue will be empty after this call returns. Elements with an
	 * unexpired delay are not waited for; they are simply discarded from
	 * the queue.
	 */
	public void clear() {
		final ReentrantLock lock = this.lock;
	 ...
	 * <tt>DelayQueue</tt> is not capacity constrained.
	 * 
	 ...
	 * The returned array elements are in no particular order. If the queue
	 ...
	 * the array has more elements than this queue), the element in the
	 * array immediately following the end of the queue is set to
	 * <tt>null</tt>.
	 * 
	 * <p>
	 ...
	 *                 supertype of the runtime type of every element in
	 *                 this queue
	 * @throws NullPointerException
	 ...
	 * unexpired) in this queue. The iterator does not return the elements
	 * in any particular order. The returned <tt>Iterator</tt> is a
