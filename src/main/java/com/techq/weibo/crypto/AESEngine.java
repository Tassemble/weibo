...
public class AESEngine implements BlockCipher {
	...
	private int inv_mcol(int x) {
		final int f2 = this.FFmulX(x);
		final int f4 = this.FFmulX(f2);
		final int f8 = this.FFmulX(f4);
		final int f9 = x ^ f8;

		return f2 ^ f4 ^ f8 ^ this.shift(f2 ^ f9, 8) ^ this.shift(f4 ^ f9, 16)
				^ this.shift(f9, 24);
	}
	...
	private int[][] generateWorkingKey(byte[] key, boolean forEncryption) {
		final int KC = key.length / 4; // key length in words
		int t;
	...
	/**
	 * initialise an AES cipher.<br>
	 * AES specified a fixed block size of 128 bits and key sizes 128/192/256
	 * bits.
	 * 
	 * @param forEncryption
	...
	public void init(boolean forEncryption, byte[] key) {
		WorkingKey = this.generateWorkingKey(key, forEncryption);
	...
	public int crypt(byte[] in, int inOff, int length, byte[] out, int outOff) {
		...
		if (in == null || out == null)
			throw new NullPointerException(
					"input or output buffer should not be null");
		if (inOff < 0 || outOff < 0)
			throw new IllegalArgumentException(
					"input or output offset should >=0");

		return this.doCrypt(in, inOff, length, out, outOff);
	}

	protected int doCrypt(byte[] in, int inOff, int length, byte[] out,
			int outOff) {

		// get source input buffer data length
		int srcLen = in.length - inOff;
		if (srcLen > length) {
			srcLen = length;
		}

		// get loops and number of bytes to padding
		int padding = 0, loops = 0;
		int remaining;
		if ((remaining = srcLen % 16) != 0) {
			padding = 16 - remaining;
			loops++;
		}
		loops += srcLen / 16;

		if (out.length - outOff < srcLen + padding)
		...
		if (padding > 0) {
			...
			in = copyOfRange(in, inOff, inOff + 16);
			final BlockPadder padder = new EmptyBytePadder();
			padder.pad(in, 16 - padding);
			inOff = 0;
		}
	...
	protected int processBlock(byte[] in, int inOff, byte[] out, int outOff) {
		...
		if (forEncryption) {
			...
			this.encryptBlock(WorkingKey);
			this.packBlock(out, outOff);
		} else {
			this.unpackBlock(in, inOff);
			this.decryptBlock(WorkingKey);
			this.packBlock(out, outOff);
		}
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		System.out.println(crypto.decrypt("TwAy6l7mEDO2pJvbsZUp0g"));
	}
}
